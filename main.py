...
class CureCell(ImplCell):
    ...
    def process(self):
        cells = self.grid.cells
        loc = self.location
        neighbors = grid.count_neighbors(loc.i, loc.j, CureCell)
        dead_neighbors = grid.count_neighbors(loc.i, loc.j, DeadCell)
        if dead_neighbors >= 6:
            return DeadCell(loc.i, loc.j, self.grid)
        if neighbors>=3:
            return DeadCell(loc.i, loc.j, self.grid)
...
grid.set_cell(AliveCell(3, 3, grid))
grid.set_cell(AliveCell(3, 4, grid))
grid.set_cell(AliveCell(3, 5, grid))
grid.set_cell(AliveCell(4, 3, grid))
grid.set_cell(AliveCell(5, 4, grid))





grid.set_cell(CancerCell(2,20,grid))



grid.set_cell(CancerCell(20,2,grid))

# print(isinstance(grid.cells[3][3], AliveCell))
runner = GameRunner(grid)
